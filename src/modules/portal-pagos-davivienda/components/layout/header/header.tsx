...
export function Header() {
  ...
  return (
    <header className="bg-brand text-white">
      <Container className="flex h-[70px] flex-wrap items-center min-[576px]:h-[117px] min-[576px]:content-center min-[768px]:h-[90px]">
        ...
        <nav className="hidden w-full min-[576px]:block min-[768px]:w-auto min-[768px]:flex-1">
          <ul className="flex items-center justify-center gap-8 min-[576px]:leading-[46px] min-[768px]:justify-end">
            {headerNav.map(({ label, href, target, rel }) => (
              <li key={label}>
                <a
                  ...
                  target={target}
                  rel={rel}
                  className="text-[14px] leading-[21px] text-white transition-colors duration-300 hover:text-[#ff9393]"
                >
                  {label}
                </a>
              </li>
